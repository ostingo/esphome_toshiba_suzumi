...
constexpr float FAN_LOW_THRESHOLD = 0.75;      // ≤ 0.75°C → LOW
constexpr float FAN_CUSTOM2_THRESHOLD = 1.0;   // ≤ 1.0°C → CUSTOM_FAN_LEVEL_2
constexpr float FAN_MEDIUM_THRESHOLD = 1.5;    // ≤ 1.5°C → MEDIUM
constexpr float FAN_CUSTOM4_THRESHOLD = 2.0;   // ≤ 2.0°C → CUSTOM_FAN_LEVEL_4
constexpr float FAN_HIGH_THRESHOLD = 3.0;      // > 2.0°C → HIGH
...
if (this->power_state_ == STATE::ON) {
  if (!isnan(this->current_temperature) && !isnan(this->target_temperature)) {
    ...
    if (temp_diff > FAN_HIGH_THRESHOLD) {
      ...
      if (this->custom_fan_mode != CUSTOM_FAN_LEVEL_4) {
        this->set_custom_fan_mode (CUSTOM_FAN_LEVEL_4);
    ...
    } else if (temp_diff > FAN_MEDIUM_THRESHOLD) {
      ...
      if (this->custom_fan_mode != CUSTOM_FAN_LEVEL_2) {
        this->set_custom_fan_mode (CUSTOM_FAN_LEVEL_2);
    ...
    } else if (temp_diff > FAN_LOW_THRESHOLD) {
    ...
    } else if (temp_diff <= FAN_QUIET_THRESHOLD) {
